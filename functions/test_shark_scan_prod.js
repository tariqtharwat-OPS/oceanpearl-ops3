...
async function getIdToken(email, password) {
    ...
    let res = await request({
        ...
        headers: {
            ...
            'Referer': 'https://oceanpearl-ops.firebaseapp.com'
        }
    }, d);
    let j = json(res);
...
// Callable functions expects JSON body { data: ... }
...
async function run() {
    console.log("🦈 SHARK AI SCAN TEST (PRODUCTION)");
    console.log("==================================");
    ...
    if (result.result && result.result.success) {
        ...
        if (result.result.alertsGenerated >= 0) {
            console.log("\n✅ SUCCESS: Scan completed in production.");
        }
    } else {
        console.log("\n❌ FAILED:");
