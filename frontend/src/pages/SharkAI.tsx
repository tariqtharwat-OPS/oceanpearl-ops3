  ...
  <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
      <div>
        ...
        <p style={{ color: '#64748b', margin: '0.5rem 0 0 0' }}>Real-time fraud detection and operational risk monitoring</p>
      </div>
      <button onClick={fetchAlerts} style={btnStyle}>{t('common_search', language)} Update</button>
    </div>
          ...
          {riskSummary ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ textAlign: 'center', padding: '1.5rem', background: '#f8fafc', borderRadius: '1rem' }}>
                  ...
                  <div style={{ fontSize: '0.875rem', fontWeight: '700', color: '#64748b' }}>GLOBAL RISK INDEX</div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                  <StatBox label="Active Alerts" value={riskSummary.activeCount} />
                  <StatBox label="Detection" value="ON" color="#10b981" />
                </div>
              </div>
    ...
    <div style={cardStyle}>
      ...
      {alerts.length === 0 ? (
        <div style={{ padding: '4rem', textAlign: 'center', color: '#94a3b8' }}>
          ...
          <p>No active alerts detected. Operations are stable.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {alerts.map(a => (
          <div key={a.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              {a.status === 'CLOSED' && (
              <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#10b981' }}>RESOLVED</span>
                      )}
            </div>
          </div>
          ...
          <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#f8fafc', borderRadius: '0.5rem', border: '1px solid #e2e8f0' }}>
            ...
            placeholder="Explain why this alert is being closed..."
            style={{ width: '100%', padding: '0.5rem', borderRadius: '0.4rem', border: '1px solid #cbd5e1', marginBottom: '1rem', height: '80px' }}
                      />
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => handleCloseAlert(a.id)} style={{ padding: '0.5rem 1rem', background: '#10b981', color: 'white', border: 'none', borderRadius: '0.4rem', fontWeight: '700', cursor: 'pointer' }}>Confirm Resolution</button>
